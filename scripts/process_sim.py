...
def integrate_power(voltage, lowco=30, hico=80, tstep=0.1e-9):
    ...
    maxfreqbin = int(
        np.floor(tstep / 5e-9 * dlength / 2.) + 1)  # Apply frequency bandpass only up to 100 MHz i.e. LOFAR maximum
    # shortspec=np.array([instr_spec[0:maxfreqbin,0]*window[0,0:maxfreqbin,0],instr_spec[0:maxfreqbin,1]*window[0,0:maxfreqbin,0]])
    shortspec = np.array(
        [instr_spec[0:maxfreqbin, 0] * window[0, 0:maxfreqbin], instr_spec[0:maxfreqbin, 1] * window[0, 0:maxfreqbin]])
...
def ProcessData(datadir, fileno, lowco, hico):
    ...
    zenith = (np.genfromtxt(re.findall("THETAP.*", open(steerfile, 'r').read()))[
        1]) * np.pi / 180.  # rad; CORSIKA coordinates
    azimuth = np.mod(np.genfromtxt(re.findall("PHIP.*", open(steerfile, 'r').read()))[1],
                     360) * np.pi / 180.  # rad; CORSIKA coordinates
    ...
    for j in np.arange(nantennas):
        ...
        coreasfile = '{0}/SIM{1}_coreas/raw_{2}.dat'.format(datadir, str(fileno).zfill(6),
                                                            antenna_file[:-1])  # drop the \n from the string!
        ...
        poldata[:, 0] = xyz_trace[j, 0] * np.cos(zen_rot) * np.cos(az_rot) + xyz_trace[j, 1] * np.cos(zen_rot) * np.sin(
            az_rot) - np.sin(zen_rot) * xyz_trace[j, 2]
        poldata[:, 1] = np.sin(az_rot) * -1 * xyz_trace[j, 0] + np.cos(az_rot) * xyz_trace[
            j, 1]  # -sin(phi) *x + cos(phi)*y in coREAS 0=positive y, 1=negative x
    ...
    antenna_position[:, 0], antenna_position[:, 1], antenna_position[:, 2] = -temp[:, 1] / 100., temp[:,
                                                                                                 0] / 100., temp[:,
                                                                                                            2] / 100.
    azimuth = 3 * np.pi / 2 + azimuth  # +x = east (phi=0), +y = north (phi=90)

    sim_info = {'e_time': e_time, 'zenith': zenith, 'azimuth': azimuth, 'energy': energy, 'hillas': hillas,
                'antenna_position': antenna_position, 'fluence': fluence, 'xyz_trace': xyz_trace,
                'poldata': onsky_trace, 'poldata_filt': trace_01, 'voltage': voltage,
